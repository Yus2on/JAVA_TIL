...
/**
 * Thread Pool
 * - 스레드를 직접 만들어 사용할 경우, 작업 마친 스레드는 종료 됨
 *      -> 멀티스레드 작업을 계속 할 경우, 스레드를 생성 / 삭제하는 오버헤드가 있다.
 *
 *  - 스레드 풀
 *      - 미리 스레드를 생성해 두고, 작업ㅁ나 스레드에 할당하여 동작
 *      - 미리 생성해 둔 스레드의 집합을 스레드 풀 이라고 한다
...
public class Main {
    public static void main(String[] args) throws InterruptedException {
         ...
         * newCachedThreadPool
         *  - 초기 스레드가 0개
         *  - 코어 스레드가 0개 (코어 스레드: 일하지 않아도 살려두는 스레드)
         *  - 요청 작업보다 스레드가 부족하면 새 스레드 생성
         *  - 60초동안 일하지 않은 스레드는 제거
         *  - 좀 안정적으로 늘렸다 줄였다 하고싶을 때 씀
         */
        ExecutorService pool2 = Executors.newFixedThreadPool(10);
        /**
         * newFixedThreadPool
         *  - 초기 스레드 nThread개에서 시작
         *  - 코어 스레드 nThread개(인자)
         *  - 작업하지 않는 스레드도 제거하지 않고 동작 ( = 한 번 증가하면 다시 줄이지 않음)
         */

        ExecutorService es = new ThreadPoolExecutor(
                10, // 코어 스레드 개수
                100, // 최대 스레드 개수
                120, // 대기 시간 -> 스레드가 이 시간동안 일하지 않으면 제거
                TimeUnit.SECONDS, // 대기 시간을 월,일,분,초 등을 정의
                new SynchronousQueue<>() // 작업 요청 -> 작업을 쌓아둘 큐에 넣어둠 -> 스레드 풀로 넘어가서 동작
        );
